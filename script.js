...
async function loadBcrypt() { if (bcryptjs) return bcryptjs; await loadScript('https://cdn.jsdelivr.net/npm/bcryptjs@2.4.3/dist/bcrypt.min.js'); bcryptjs = window.dcodeIO.bcrypt; return bcryptjs; }
async function loadArgon2() { if (argon2js) return argon2js; await loadScript('https://cdn.jsdelivr.net/npm/argon2-browser/lib/argon2.min.js'); argon2js = window.argon2; return argon2js; }
...
async function handleGenerateClick() {
    ...
    showLoader(true);
    try {
        if (algorithm === 'Bcrypt') { const bcrypt = await loadBcrypt(); await handleBcrypt(bcrypt); }
        else if (algorithm === 'Argon2') { const argon2 = await loadArgon2(); await handleArgon2(argon2); }
        else { handleStandardHash(); }
    } catch (error) {
        alert(error.message);
    } finally {
        if (algorithm !== 'Bcrypt' && algorithm !== 'Argon2') { showLoader(false); }
    }
}
...
async function handleBcrypt(bcrypt) {
    ...
    if (selectedMode === 'hash') {
        ...
        bcrypt.hash(plaintext, costFactor, (err, hash) => { showLoader(false); if(err){alert('Error: '+err); return;} document.getElementById('hash-output').value = hash; });
    } else {
